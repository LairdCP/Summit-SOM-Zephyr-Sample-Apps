...
void main(void)
{
	...
	ret = gpio_pin_configure(red_dev, RED_PIN,
							 GPIO_OUTPUT_ACTIVE | RED_FLAGS);
	if (ret < 0) {
	...
	ret = gpio_pin_configure(green_dev, GREEN_PIN,
							 GPIO_OUTPUT_ACTIVE | GREEN_FLAGS);
	if (ret < 0) {
	...
	ret = gpio_pin_configure(blue_dev, BLUE_PIN,
							 GPIO_OUTPUT_ACTIVE | BLUE_FLAGS);
	if (ret < 0) {
